  ...
  return (
    <View style={[badge.wrap,
      ...
      <View style={[badge.dot,
        { backgroundColor: open ? colors.openBadge : colors.closedBadge }]} />
...
function RestaurantCard({
  ...
  return (
    <TouchableOpacity
      style={[cs.card, { backgroundColor: colors.card, shadowColor: colors.shadow }]}
      onPress={onPress} activeOpacity={0.88}>
      <View style={cs.imgWrap}>
        {src
          ? <Image source={src} style={cs.img} resizeMode="cover" />
      ...
      <View style={cs.body}>
        ...
        <View style={cs.tagRow}>
          {restaurant.tags.slice(0, 2).map(t => (
            <View key={t} style={[cs.tagChip, { backgroundColor: colors.tagBg }]}>
              <Text style={[cs.tagChipTxt, { color: colors.tagText }]}>{t}</Text>
            </View>
          ))}
        </View>
      </View>
...
function ImageGallery({
  images, colors, onAllImages,
}: { images: string[]; colors: ColorScheme; onAllImages: () => void }) {
    ...
    <View style={gs.wrap}>
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return src
            ? <Image source={src} style={[gs.img, { width }]} resizeMode="cover" />
            : <View style={[gs.img, { width, backgroundColor: colors.primaryLight },
                gs.placeholder]}>
                <Text style={{ fontSize: 50 }}>🍗</Text>
              </View>;
        }}
      />
      <TouchableOpacity style={gs.allBtn} onPress={onAllImages}>
        <Text style={gs.allBtnTxt}>⊞ 所有圖片</Text>
      </TouchableOpacity>
      <View style={gs.counter}>
        <Text style={gs.counterTxt}>{current + 1} / {images.length}</Text>
      </View>
      <View style={gs.dots}>
        {images.map((_, i) => (
          <View key={i} style={[gs.dot,
            i === current
              ? [gs.dotActive, { backgroundColor: colors.primary }]
              : gs.dotInactive]} />
        ))}
      </View>
    </View>
...
const gs = StyleSheet.create({
  wrap:       { position: 'relative', height: 260 },
  img:        { height: 260 },
  placeholder: { justifyContent: 'center', alignItems: 'center' },
  allBtn: {
  ...
  counter:    { position: 'absolute', top: 12, left: 12,
    backgroundColor: 'rgba(0,0,0,0.55)', borderRadius: 10,
  ...
  dotActive:  { width: 18 },
  dotInactive: { width: 7, backgroundColor: 'rgba(255,255,255,0.5)' },
});

// ─── 主畫面 ────────────────────────────────────────────────
export default function RestaurantInfoScreen({ navigation }: Props) {
  ...
  const filtered = useMemo(() =>
    restaurants.filter(r => {
      ...
      return true;
    }), [appliedSearch, filterOpen, filterDay]);
        ...
        <View style={[ms.sheet, { backgroundColor: colors.surface }]}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                <TouchableOpacity
                  style={[ms.item, { borderBottomColor: colors.divider },
                    selected?.id === item.id && { backgroundColor: colors.primaryLight }]}
                  onPress={() => { setSelected(item); setDropdownVisible(false); }}>
                  {src
                    ? <Image source={src} style={ms.thumb} />
                    : <View style={[ms.thumb, { backgroundColor: colors.primaryLight,
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={[ms.itemAddr, { color: colors.textSecondary }]}
    ...
    return (
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        <ImageGallery
          ...
          colors={colors}
          onAllImages={() => navigation.navigate('ImageOverview', { restaurant: selected })}
        />
        <View style={[ds.pad, { backgroundColor: colors.background }]}>
          <View style={ds.header}>
            ...
            <View style={{ alignItems: 'flex-end', gap: 6 }}>
              ...
              <TouchableOpacity onPress={() => toggleFavorite(selected.id)}>
                <Text style={{ fontSize: 26 }}>{fav ? '❤️' : '🤍'}</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={ds.actionRow}>
            <TouchableOpacity
              style={[ds.actionBtn, { backgroundColor: colors.primary, flex: 1 }]}
            ...
            <TouchableOpacity
              style={[ds.actionBtn, { backgroundColor: colors.accent, width: 56 }]}
    ...
    <SafeAreaView style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.appBar, { backgroundColor: colors.header }]}>
        ...
        {selected && (
          <TouchableOpacity onPress={() => setSelected(null)} style={styles.iconBtn}>
            <Text style={{ color: colors.headerText, fontSize: 13 }}>全部</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={[styles.searchRow, { backgroundColor: colors.header }]}>
        <View style={[styles.searchBox, { backgroundColor: colors.inputBg }]}>
          ...
          <TextInput
            value={search} onChangeText={setSearch} onSubmitEditing={handleSearch}
            placeholder="搜尋店名或地址..."
        ...
        <TouchableOpacity
          ...
          <Text style={{ color: '#FFF', fontWeight: 'bold', fontSize: 13 }}>搜尋</Text>
        </TouchableOpacity>
      </View>

      {/* 篩選列 */}
      <View style={[styles.filterRow, { backgroundColor: colors.surface, borderBottomColor: colors.divider }]}>
        <FlatList
          horizontal showsHorizontalScrollIndicator={false}
          data={[{ key: 'open', label: '🟢 營業中' },
            ...DAY_LABELS.map((l, i) => ({ key: `day${i}`, label: `週${l}`, dayIdx: i }))]}
            ...
            return (
              <TouchableOpacity
                style={[styles.chip, { borderColor: colors.border },
                  isActive && { backgroundColor: isOpenFilter ? colors.primary : colors.secondary,
                    borderColor: isOpenFilter ? colors.primary : colors.secondary }]}
                onPress={() => {
                ...
                <Text style={[styles.chipTxt,
                  { color: isActive ? (isOpenFilter ? '#FFF' : colors.accent) : colors.text }]}>
                  {item.label}
                </Text>
              </TouchableOpacity>
            );
      ...
      <TouchableOpacity
        ...
        <Text style={[styles.dropdownTxt,
          { color: selected ? colors.text : colors.textLight }]} numberOfLines={1}>
      ...
      {selected ? renderDetail() : (
        <FlatList
          ...
          numColumns={2}
          contentContainerStyle={{ padding: 12, paddingTop: 4, gap: 10 }}
          ...
          renderItem={({ item }) => (
            <RestaurantCard restaurant={item} colors={colors} onPress={() => setSelected(item)} />
          )}
        />
      )}
...
const ms = StyleSheet.create({
  ...
  sheet:   { borderTopLeftRadius: 20, borderTopRightRadius: 20,
    paddingTop: 12, paddingBottom: 30, maxHeight: '80%' },
